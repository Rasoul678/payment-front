...
/**
 * Highlights a given string with the provided highlight term.
 *
 * @param text - The text to be highlighted.
 * @param highlight - The term to highlight within the text.
 * @returns A React element with the highlighted text.
 */
...
const PaymentList: React.FC<IProps> = () => {
  ...
  const { data, isLoading, isFetching, refetch, isPlaceholderData } =
    useQuery<PaymentsQueryType>({
  ...
  return (
    <section>
      ...
      <div className="flex flex-col justify-between min-h-[calc(100vh-11rem)]">
        {(isLoading || isFetching) && !payments && <Spinner />}
        {err && !isFetching && (
        ...
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 items-center content-start gap-4 mt-2 mb-[1rem] ${
                  ...
                  <CardHeader className="p-3 text-justify border-0 border-b-2">
                    <CardTitle>
                      <div className="relative inline-flex">
                        ...
                        />
                        <span className="capitalize ml-2">{pay.status}</span>
                      </div>
                    </CardTitle>
                    <CardTitle className="capitalize">
                      Type: {pay.type}
                    </CardTitle>
                    <CardTitle>Amount: {formatCurrency(pay.value)}</CardTitle>
                  </CardHeader>
